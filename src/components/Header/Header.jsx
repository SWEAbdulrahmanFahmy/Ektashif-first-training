...
const Header = () => {
  ...
  return (
    <header className={`bg-white py-2 px-12 sticky top-0 z-50 shadow-md flex flex-row-reverse justify-between h-[62px] ${loaded ? 'animate-header' : ''}`}>
      ...
      {/* Dropdown */}
      {isOpen && (
        <div className="absolute z-10 left-0 mt-2 w-56 bg-white rounded-lg shadow-lg">
          <ul className="py-1">
            <li className="block px-4 py-2 text-gray-800 text-center hover:bg-gray-200">
              <a href="#">الرئيسية</a>
            </li>
            <li className="block px-4 py-2 text-gray-800 text-center hover:bg-gray-200">
              <a href="#">الحجوزات</a>
      ...
      {/* nav */}
      <nav className="hidden lg:flex gap-10 flex-row-reverse pt-3">
      ...
      {/* language */}
      <div className="hidden lg:flex gap-2 flex-row pt-3 hover:text-[#2B49FF]">
